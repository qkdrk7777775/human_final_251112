...
def get_reported_posts_by_id(post_id:int, user_id:str):
    query = text("SELECT * FROM reported_posts WHERE post_id = :post_id AND user_id = :user_id")
    with engine.connect() as conn:
...
def get_reported_posts_by_post_id(post_id:int):
    query = text("SELECT * FROM reported_posts WHERE post_id = :post_id")
    with engine.connect() as conn:
...
def get_reported_posts():
    query = """
        ...
        JOIN post p ON rp.post_id = p.id
    """
    query = text(query)
...
# 신고 게시글 삭제
def delete_post_report_by_id(post_id: int, user_id:int):
    query = text("DELETE FROM reported_posts WHERE post_id=:post_id AND user_id = :user_id")
    print(query,post_id,user_id)
    with engine.connect() as conn:
        result = conn.execute(query, {"post_id": post_id, "user_id":user_id})
        conn.commit()
...
def delete_post_report_by_post_id(post_id: int):
    query = text("DELETE FROM reported_posts WHERE post_id=:post_id")
    with engine.connect() as conn:
        result = conn.execute(query, {"post_id": post_id})
        conn.commit()
        deleted = result.rowcount > 0
